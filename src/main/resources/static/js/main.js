...
function triggerPolling() {
	...
	fetch('/service/' + 'checkAndUpdateServiceStatus/', {
		method: 'POST',
		headers: {
	...
	fetch(servicesRequest)
		...
		.then(function(serviceList) {
			...
			serviceList.forEach(service => {
				...
				li.appendChild(document.createTextNode(service.name + ': ' + service.url + ' (' + service.status + ') ' + 'createdTime: ' + service.addedTime + ' updatedTime: ' + service.updatedTime));
...
const updateButton = document.querySelector('#put-service');
updateButton.onclick = evt => {
	let urlName = document.querySelector('#url-name').value;
	let serviceName = document.querySelector('#service-name').value;
	if (document.querySelector('#fail').checked) {
		status = 'FAIL';
	}
	else {
		status = 'OK';
	...
	fetch('/service', {
		...
		body: JSON.stringify({ name: serviceName, url: urlName, status: status })
	});
};
